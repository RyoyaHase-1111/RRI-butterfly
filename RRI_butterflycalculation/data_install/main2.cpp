...
void initialize()
{
	...
	if (fp) {
		...
		data = (double*)malloc(sizeof(double) * datastep); //動的メモリの確保
	...
	if (fp) {
		...
		for (i = 0; i < datastep; i++) {
			fscanf(fp, "%lf\n", &data[i]);
			if (i < samples) { fftbuf[0][i] = (data[i] * 0.1)* fft.window_func(WF_Rectangular, i, samples); fftbuf[1][i] = 0; }
			if (i > 0) {
				plot[1] = plot[0];
				plot[0] = alfa * data[i];
	...
	fft.fft_cal(fftbuf[0], fftbuf[1], spbuf[0], spbuf[1], 7);

	double ps[2], f, LF = 0, HF = 0;
	ps[1] = sqrt(spbuf[0][0] * spbuf[0][0] + spbuf[1][0] * spbuf[1][0])*100.0;
	for (i = 1; i < samples/2; i++) {
		ps[0] = sqrt(spbuf[0][i] * spbuf[0][i] + spbuf[1][i] * spbuf[1][i])*100.0;
		if (i > 2) {
			f = (double)i / (0.5 * samples);
