...
void initialize()
{
	...
	const int samples = 8192, bitSize = 13;
	int i, j, k;
	const int buf_size = 100;
	char buf[buf_size];
	double *fftbuf[2];
	double *spbuf[2];
	double interval = 500.0 / (double)samples, tsum = 0;
	...
	double plot[2];
	double alfa = 200.0 / 1.5, beta = 10.0;
	...
	if (fp) {
		...
		for (i = 0; i < datastep; i++) {
			...
			if (i > 0) {
				gamma[1] = gamma[0];
				if (fabs(data[1][i] - data[1][i - 1]) > 0.25)
					gamma[0] = 0.05 / fabs(data[1][i] - data[1][i - 1]);
				else
					gamma[0] = 1.0;
				data[1][i] = (gamma[0] * (data[1][i] - data[1][i - 1]) + data[1][i - 1]);
